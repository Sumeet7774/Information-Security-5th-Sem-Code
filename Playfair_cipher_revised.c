...
void rotateMatrix90DegreesClockwise(char matrix[5][5], char rotatedMatrix[5][5]) {
    int i, j;
    for (i = 0; i < 5; i++) {
        for (j = 0; j < 5; j++) {
...
int main() {
    ...
    rotateMatrix90DegreesClockwise(keyMatrix, rotatedMatrix);
    printf("Rotated 90 Degrees Clockwise Key Matrix:\n");
    displayKeyMatrix(rotatedMatrix);

    prepareText(text, preparedText);
    encryptText(preparedText, rotatedMatrix);
